...
export const apiLintTypesTool = defineTool({
  ...
  description:
    "Deterministically lint Go API type files against Kubernetes/OpenShift API conventions (openshift/enhancements CONVENTIONS + k8s api-conventions): json tag naming, +optional/+required markers, omitempty consistency, bare/unsigned ints, float in APIs, bool pointers, List items, TypeMeta/ObjectMeta pairing, +union discriminators, printcolumns, package version naming, deprecated markers. Pass a types_*.go file or a package directory. No Go toolchain required.",
